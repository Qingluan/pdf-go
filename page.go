...
func (p Page) GetImgs() (imgs []io.ReadCloser, err error) {
	...
	// fmt.Println(p.V.Key("Subtype"))
	return
}

func (p Page) GetText() (result string, err error) {
	fonts := make(map[string]*Font)
...
func (p Page) GetPlainText(fonts map[string]*Font) (result string, err error) {
	...
	Interpret(strm, func(stk *Stack, op string) {
		...
		case "Tm":
			...
			// x := args[4].Float64()
			y := args[5].Float64()
			if y != last_y && last_y != 0 {

				// showText("\n")
				if lst_sep != 0 {
					if y-last_y > lst_sep {
						line_num := int((y - last_y) / lst_sep)
				...
				// fmt.Println("line y:", y, " last y:", last_y, " left:", y-last_y)
			}

			// fmt.Println("\n[count] :", count, " width : ", x-last_x, y)
			count = 0
			// last_x = x
			last_y = y
